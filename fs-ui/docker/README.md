...
以下命令均在**项目根目录**下执行，将 `http://your-api` 替换为你的实际后端地址。

```bash
# 1. 构建镜像
docker build -f docker/Dockerfile -t gfs-frontend .

# 2. 启动容器
docker run -d -p 80:80 \
  -e API_BASE_URL=http://your-api \
  --name gfs-frontend \
  gfs-frontend
```
...
docker build -f docker/Dockerfile \
  --build-arg VITE_API_BASE_URL=http://your-api \
  -t gfs-frontend .
```
