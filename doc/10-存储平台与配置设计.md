...
- **挂载式存储（LocalMount / SMB）**：一种特例。它不是「网盘向存储搬文件」，而是**把真实文件系统/网络共享镜像成网盘的目录树**：`MountScanService` 定时扫描真实目录，双向比对后把变化同步回 `file_info`（新增导入、变化更新、消失硬删）。哪些平台是挂载式由能力位 `isMountMode()` 声明（LocalMount、SMB），不绑定平台名单。
...
deleteStorageSettingById
  ...
  ▼ 删行 + removeInstance + （isMountMode 平台触发挂载卸载回调）
```
...
@Scheduled(5min) / 启动后  MountScanService.scanAll
  ▼ 遍历所有启用挂载能力位（isMountMode）的配置 → scanSetting（全程持 MountLocks，与写穿透互斥）
  ├─ 实例不可用 / 非挂载 → 本轮放弃
  ├─ 根目录列举失败 → 本轮放弃（绝不误删）
...
```mermaid
flowchart TD
    A["@Scheduled(5min) / 启动后<br/>scanAll 遍历启用挂载能力位的配置"] --> B["scanSetting<br/>全程持 MountLocks（与写穿透互斥）"]
    B --> C{"实例可用且为挂载式?"}
