# GFS 对外文件服务设计（WebDAV / SFTP / FTP / OSS，SPI 插件式）

> 面向读者：想理解 GFS 如何把私有网盘暴露为标准协议（WebDAV / SFTP / FTP / S3 兼容 OSS），以及如何以 **SPI 插件方式新增协议**的开发者。
>
...
- **OSS（S3 兼容）**：运行在 `/oss/**`，把整个网盘暴露为 S3 的虚拟桶 `gfs`（path-style），自实现 **AWS SigV4 验签**（不依赖 AWS SDK），支持 rclone / AWS SDK / Cyberduck / WinSCP S3 等客户端与分片上传三件套。
...
| OSS | `OssController` | `fs-service/.../oss` | `/oss/**` 单控制器：ListBuckets / ListObjects / Get / Put / Delete / 分片三件套按 method + 子资源分发 |
| OSS | `OssAuthFilter` | `fs-service/.../oss` | 服务启用开关 + SigV4 验签 + Sa-Token 桥接（XML 错误语义） |
| OSS | `OssSigV4Verifier` | `fs-service/.../oss` | AWS SigV4 自实现：HMAC 推导链 + Canonical Request 重建（无 AWS SDK） |
| OSS | `OssAesCipher` | `fs-service/.../oss` | Secret Key 存储加密（AES-256-GCM，主密钥 `oss.aes-key`） |
| OSS | `OssAccessKeyService` / `OssAccessKeyController` | `fs-service/.../oss` + `controller` | 每用户 SigV4 密钥对签发（AK 20 位 / SK 40 位，明文仅返回一次）/ 列表 / 吊销 |
| OSS | `OssMultipartRegistry` | `fs-service/.../oss` | 分片上传会话（内存态）：分片落临时文件，Complete 合并，TTL 2h |
| OSS | `OssExternalService` / `OssFilterConfig` | `fs-service/.../oss` | SPI 注册（socketBased=false）+ 过滤器注册（order -100） |
...
```
客户端（rclone / AWS SDK / Cyberduck…）→ /oss[/gfs/key]
  ▼ OssAuthFilter（order -100，只拦 /oss/*）
    ├─ 服务未启用 → 503 ServiceUnavailable
    ├─ 无 Authorization → 403 AccessDenied（拒绝匿名）
    ├─ AK 不存在/已吊销/解密失败 → 403 InvalidAccessKeyId
    └─ SigV4 验签失败 → 403 SignatureDoesNotMatch
    └─ 验签通过 → davUserAuthenticator.bridgeSaToken(userId)
  ▼ OssController.handle()
    ├─ GET  /oss/                          → ListBuckets（固定虚拟桶 gfs）
    ├─ GET  /oss/gfs?list-type=2&prefix=…  → ListObjects（递归展开目录树，服务端 prefix 过滤）
    ├─ GET  /oss/gfs/a/b.txt               → 对象读取（Range → 206）
    ├─ PUT  /oss/gfs/a/b.txt               → 上传/覆盖（父目录自动逐级创建）
    ├─ DELETE /oss/gfs/a/b.txt             → 进回收站（S3 幂等：不存在也 204）
    ├─ POST ?uploads / PUT ?partNumber&uploadId / POST ?uploadId → 分片三件套
    └─ 未知桶名 → 404 NoSuchBucket
```

**三个设计决策**：

1. **SigV4 验签必须还原 Secret Key 明文**（HMAC 推导链 `kSigning = HMAC(kService=…)` 需要 SK 参与），BCrypt 不可逆用不了 → 密钥表用 **AES-256-GCM 可逆加密**存储（`OssAesCipher`，密文 `Base64(iv‖ciphertext+tag)`，主密钥 `oss.aes-key` 32 字节）；主密钥丢失仅影响已签发 SK（重新签发即可），不波及账号密码。
2. **虚拟桶 + 目录映射**：整个网盘是 S3 的一个桶 `gfs`，对象 key = 网盘相对路径；S3 无目录语义，`PUT` 时父目录自动逐级创建，`ListObjects` 递归展开目录树后按 prefix 服务端过滤（单次上限 10000 条）。
3. **分片上传走内存会话**：`OssMultipartRegistry` 把分片落临时文件，Complete 时按 partNumber 升序合并为单一流交给 `writeFileContent` 走正常入库/去重链路；孤儿会话 2 小时 TTL 清理。

**密钥管理**：每用户可签发多对 SigV4 密钥（`/apis/service/oss/keys`，登录态）——AK 20 位（`GFS` 前缀）对外可见唯一，SK 40 位随机仅创建响应明文返回一次；吊销（status=1）立即失效；鉴权成功节流刷新 `last_used_at`。
...
| OSS 验签 | 自实现 SigV4（不引 AWS SDK） | 验签只需 HMAC-SHA256 推导链与 Canonical Request 重建，百行内可控；引 SDK 反而带一堆用不上的依赖与版本约束 |
| OSS SK 存储 | AES-256-GCM 可逆加密（`oss.aes-key`） | SigV4 验签需还原 SK 明文参与 HMAC，BCrypt 不可逆；GCM 带认证标签，篡改/换密钥直接解密失败 |
| OSS 挂载点 | 复用主 HTTP 端口（`/oss/**`，socketBased=false） | S3 客户端都支持配置路径前缀，独立端口反而要额外处理 CORS/证书，与 WebDAV 同构 |
...
- **OSS 网关边界**：固定单桶 `gfs`（不做桶管理/ACL/预签名 URL/POST 表单上传/`?delete` 批量删）；不支持匿名访问（必须 SigV4 签名）；`oss.aes-key` 缺失时启动随机生成并告警（重启后历史 SK 失效需重签）；ListObjects 无分页 token（单次上限 10000，超大网盘按 prefix 分前缀遍历）；密钥管理界面按用户隔离，管理员同样只管自己的密钥。
...
| OSS 网关 | `fs-modules/fs-service/.../oss/OssController.java`、`OssAuthFilter.java` |
| OSS 验签/加密 | `fs-modules/fs-service/.../oss/OssSigV4Verifier.java`、`OssAesCipher.java` |
| OSS 密钥 | `fs-modules/fs-service/.../oss/OssAccessKeyService.java`、`.../controller/OssAccessKeyController.java` |
...
- **协议即翻译器**：WebDAV/SFTP/FTP/OSS 都只是把「协议请求」翻译成对 `FileInfoService` 的业务调用，不重写业务，天然继承回收站/去重/加密/挂载语义。
...
- **诚实的能力边界**：LOCK 只做到「最小内存写锁」（不持久化/不跨实例），`PROPPATCH` 属性、跨存储协议切换不予实现；OSS 不做桶管理/ACL/匿名访问/预签名 URL，文档明确写出而非假装支持——这对协议类代码很重要。
