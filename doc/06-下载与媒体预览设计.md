...
#### 11.2.1 http 环境没有 OPFS，为什么进度条依然在

进度不是来自「字节存到了哪」，而是来自「第 N 片有没有到 JS 手里」——与存储介质完全无关。http 无 OPFS 时改变的只是 `writeChunk` 的目的地，进度产生链路一步未变：

```
downloadChunk (responseType:'blob')      ← ① 分片响应完整流经 JS
  const blob = response.data             ← ② 此刻这片已经在应用手里
  downloadedChunks.add(chunkIndex)       ← ③ 进度记账：与存储介质无关
  notifyProgress(...)                    ← ④ 页面进度条从此驱动
  writeChunk:
    有 OPFS handle → 写磁盘临时文件
    没有（http）  → memoryChunks.set(chunkIndex, blob)  ← 唯一区别：字节改住 JS 堆
```

三种模式的进度可见性：

| 模式 | 分片经过 JS？ | 页面内进度 | 字节驻留位置 |
|---|---|---|---|
| HTTPS + OPFS | ✅ | ✅ | 磁盘临时文件 |
| http ≤512MB 内存模式 | ✅ | ✅（同一个 downloadedChunks 计数） | JS 堆（Blob Map） |
| http >512MB 流式直下 | ❌ 整个文件绕过 JS | ❌ 唯一无进度路径 | 浏览器网络栈直落盘 |

即内存模式不是「没有 OPFS 就没有进度」，而是「没有 OPFS 就改用内存当仓库」——仓库换了，记账本没换。代价是全部分片 Blob 驻留标签页堆内存（组装时再翻倍），这正是 512MB 上限的由来；流式直下则是把数据流交还浏览器换无上限，自然也就交出了进度。
